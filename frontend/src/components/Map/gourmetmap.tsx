...
const GourmetMap : React.FC<GourmetMapProps> = (props) => {
    ...
    function calcCenterPositon(markers: Array<MarkerType>) {
        let lat = 0;
        let lng = 0;
        if (markers.length < 1) { return L.latLng([lat, lng])};
    ...
    useEffect(() => {
        ...
        if (newShops.length !== oldShops.length && JSON.stringify(objectSort(newShops)) !== JSON.stringify(objectSort(oldShops))) {
            const markers = getMarkerInfo(props.parentShops);
            props.setMarkers(() => {return markers});
            setOldShops(() => {return newShops;});
            setCenterPosition(() => {return calcCenterPositon(markers)});
